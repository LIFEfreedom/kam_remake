...
type
  // Dismiss is only supported for units that are not inside a house: TKMTaskDismiss.FindSchool
  // routes from the unit position, which is the unwalkable house tile while he sits inside, so no
  // school can ever be found and the task is dropped right away. The recruit must then be left
  // completely alone - alive, inside, and still listed by the barracks exactly once
...
begin
  ...
  // Let him act on the dismiss request (or rather fail to) for a while
  Result := aTick < SETTLE_TICKS + 200;
end;
...
begin
  ...
  AssertTrue(not fDismissStarted, 'Dismiss should not even start for a recruit sitting inside the barracks');
  AssertTrue(not fRecruit.IsDead, 'Recruit should be left alone when the dismiss could not start');
  AssertTrue(fRecruit.InHouse = fBarracks, 'Recruit should still be inside the barracks');
  AssertEquals(1, fBarracks.RecruitsCount, 'Barracks should still list the recruit exactly once');
end;
...
begin
  Result := 'Dismiss request for a recruit sitting inside the barracks should be dropped, leaving him listed there.';
end;
